...
#	done
	...
	for PKG in zlib m4 bison flex libelf binutils gmp mpfr mpc gcc attr acl libcap sed pkgconf ncurses util-linux iana-etc libtool iproute2 bzip2 perl readline autoconf automake bash bc file gawk grep less kbd make xz kmod expat patch gperf busybox xfsprogs $BOOTPKG; do
		buildpkg $REPO/$PKG $PKGS/$PKG
	done
}
