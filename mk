...
bootstrap_rootfs() {
	...
	pkginstall zlib m4 bison flex libelf binutils gmp mpfr mpc isl gcc attr acl libcap pkgconf ncurses util-linux e2fsprogs libtool perl readline autoconf automake bash bc file kbd make xz patch busybox libressl ca-certificates dosfstools linux libnl wpa_supplicant curl wget libarchive git npkg prt-get
}

generate_stage_archive() {
	printmsg "Building stage archive"

	pkginstallstage filesystem linux-headers musl zlib m4 bison flex libelf binutils gmp mpfr mpc isl gcc attr acl libcap pkgconf ncurses util-linux e2fsprogs libtool perl readline autoconf automake bash bc file kbd make xz patch busybox libressl ca-certificates dosfstools linux curl wget libarchive git npkg prt-get
...
generate_initrd() {
	...
	pkginstallinitrd filesystem linux-headers musl zlib attr acl libcap ncurses util-linux e2fsprogs readline bash file kbd xz busybox libressl ca-certificates dosfstools linux libnl wpa_supplicant curl wget
