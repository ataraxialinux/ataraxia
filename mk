...
build_rootfs() {
	for PKG in zlib m4 bison flex libelf binutils gmp mpfr mpc gcc pkgconf ncurses util-linux e2fsprogs libtool perl readline autoconf automake bash bc file less kbd make xz expat libressl patch busybox libarchive dropbear curl git pacman rsync linux grub; do
		install_target $PKG
	done
}
