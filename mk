...
build_rootfs() {
	...
	pkginstall zlib m4 bison flex libelf binutils gmp mpfr mpc isl gcc attr acl libcap pkgconf ncurses util-linux e2fsprogs libtool perl readline autoconf automake bash bc file kbd make xz patch busybox libressl ca-certificates linux sudo rsync ccache openssh curl libarchive pkgutils expat git go libffi python $BOOTLOADER
}
