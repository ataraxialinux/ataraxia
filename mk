...
build_rootfs() {
	for PKG in zlib m4 bison flex libelf binutils gmp mpfr mpc isl gcc pkgconf ncurses util-linux e2fsprogs libtool perl readline autoconf automake bash bc file kbd make xz kmod expat libressl ca-certificates libffi python patch gperf eudev busybox libarchive dropbear libnl-tiny wireless_tools wpa_supplicant curl git pacman rsync linux grub; do
		install_target $PKG
	done
}
